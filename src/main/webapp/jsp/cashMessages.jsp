...
<div class="layui-layout-admin" style="height:100%;padding:5px;">
    <form class="layui-form" action="${basePath}/init/cashbuildPage" name="fm" id="fm" method="post">
        <table class="layui-table" id="tab">
            ...
            <c:forEach items="${sessionScope.pagination.resultList}" var="build" varStatus="index">
                <tr>
                    ...
                    <td><code:name codeCode="${build.PROGRESS}" codeType="Progress"/></td>
                    <td>${build.FINISHAMOUNT}</td>
                    <td>${build.YETCASHAMOUNT}</td>
                    <td title="<fm:formatDate value="${build.PAYMENTDATE}" pattern="yyyy-MM-dd"/>"><fm:formatDate value="${build.OPERATETDAT}" pattern="yyyy-MM-dd"/></td>
