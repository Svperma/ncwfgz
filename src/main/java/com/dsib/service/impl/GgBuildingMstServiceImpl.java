...
public class GgBuildingMstServiceImpl implements GgBuildingMstService {
    ...
    @Override
    public Map<String, String> initData2(Map<String, Object> map) throws Exception {
        return  ggBuildingmstMapper.getGgBuildingmstCount(map);
    }
...
//        }
    ...
    @Override
    public void updateGgBuildingMstByBank(String id, String bank, String account) {
        boolean flag = true;
        GgBuildingmst building = ggBuildingmstMapper.selectByPrimaryKey(id);
        if(!building.getAccount().equals(account)){
            building.setAccount(account);
            flag = false;
        }
        if(!building.getBank().equals(bank)){
            building.setBank(bank);
            flag = false;
        }

        if(!flag) {
            ggBuildingmstMapper.updateByPrimaryKey(building);
        }
    }

    @Override
    public void insertAll(List<GgBuildingmst> list) {
        ggBuildingmstMapper.insertAll(list);
    }
