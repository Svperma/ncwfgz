...
layui.config({
	...
	if(hiddenMenu != null){
		...
		var navArray = menuVal.split(";");
		var dataArray = new Array();
		for(var i=0;i<navArray.length-1;i++){
			var dataItem = eval("("+navArray[i]+")");
			var spread = dataItem.spread;
			if(spread == "true"){
				dataItem.spread = true;
			}else if(spread == "false"){
				dataItem.spread = false;
			}
			var child = dataItem.children;
			if(child != null){
				child = child.substring(1,child.length-1);
				var childArray = new Array();
				var splitArray = child.split("*");
				for(var n = 0;n<splitArray.length;n++){
					var childItem = eval("("+splitArray[n]+")");
					childArray[n] = childItem;
				}
				dataItem.children = childArray;
			}
